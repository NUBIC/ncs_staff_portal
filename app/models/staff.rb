...
class Staff < ActiveRecord::Base
  ...
  validate :has_roles
  def has_roles
    errors.add(:roles, "can not be empty when user have #{NcsNavigator.configuration.study_center_username}.User must have atleast one role assigned. Please select one or more roles.") if self.staff_roles.blank? && !self.username.blank?
  end
