...
module NcsNavigator::StaffPortal::Warehouse::ThreePointOne
	class Enumerator
      ...
      %Q{
        SELECT
          ...
          CASE
            ...
            ELSE
              CASE
                WHEN oss.ncs_ssu_id IS NOT NULL THEN ('staff_portal-' || ot.outreach_event_id || '-' || (SELECT ssu_id FROM ncs_ssus WHERE id = oss.ncs_ssu_id) || '-#{id_prefix}' || ot.id)
          ...
          CASE
            ...
            ELSE
              CASE
                WHEN oss.ncs_ssu_id IS NOT NULL THEN ('staff_portal-' || ot.outreach_event_id || '-'|| (SELECT ssu_id FROM ncs_ssus WHERE id = oss.ncs_ssu_id))
        ...
        FROM #{table_name} ot
          LEFT JOIN (
            ...
            FROM outreach_segments WHERE ncs_ssu_id IS NOT NULL
          ) oss ON ot.outreach_event_id=oss.outreach_event_id
      ...
      :query => %Q(
        SELECT
          CASE
            ...
            ELSE
              CASE
                WHEN oss.ncs_ssu_id IS NOT NULL THEN ('staff_portal-' || oe.id || '-' || (SELECT ssu_id FROM ncs_ssus WHERE id = oss.ncs_ssu_id))
          ...
          END AS ssu_id,
          CASE 
            WHEN ost.ncs_tsu_id IS NOT NULL THEN (SELECT tsu_id FROM ncs_tsus WHERE id = ost.ncs_tsu_id)
          END AS tsu_id,
        ...
        FROM outreach_events oe
         LEFT JOIN (
           SELECT outreach_event_id, ncs_ssu_id
           FROM outreach_segments WHERE ncs_ssu_id IS NOT NULL
         ) oss ON oe.id=oss.outreach_event_id
         LEFT JOIN (
           SELECT outreach_event_id, ncs_tsu_id
           FROM outreach_segments WHERE ncs_tsu_id IS NOT NULL
         ) ost ON oe.id=ost.outreach_event_id
         LEFT JOIN (
